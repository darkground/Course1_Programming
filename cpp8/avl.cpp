...
void printAVL(AVLNode* root, std::ostream& os, const std::string& rpf, const std::string& mpf, const std::string& lpf) {
	if (root != NULL) {
        if (root->right)
            printAVL(root->right, os, rpf + "   ", rpf + ".-->", rpf + "|  ");
        os << mpf << root->value << std::endl;
        if (root->left)
            printAVL(root->left, os, lpf + "|  ", lpf + "`-->", lpf + "   ");
    }
}
